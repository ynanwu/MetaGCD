...
def split_cluster_acc_v1(y_true, y_pred, mask, stage):
...
def split_cluster_acc_v2(y_true, y_pred, mask, stage):
    """
...
def log_accs_from_preds(y_true, y_pred, mask, stage, eval_funcs: List[str], save_name: str, T: int=None, writer: SummaryWriter=None,
                        print_output=False):
    ...
    for i, f_name in enumerate(eval_funcs):
        ...
        if stage == 'pretrain':
            old_acc = acc_f(y_true, y_pred, mask, stage)
            if i == 0:
                to_return = (old_acc)

        else:
            all_acc, old_acc, new_acc = acc_f(y_true, y_pred, mask, stage)
            if i == 0:
