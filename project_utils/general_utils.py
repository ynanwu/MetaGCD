...
def hierarchical_print(in_str, length, mode, level):  # mode in ['out', 'in'], level starts from 0
    if mode not in ['out', 'in']:
        raise Exception('mode must be in [\'out\', \'in\']')
    if mode == 'out':
        out_str = '| ' * level + '+ ' + in_str + ' '
    else:
        out_str = '| ' * level + '- ' + in_str + ' '
    cur_len = len(out_str)
    if cur_len > length:
        raise Exception('length must be set larger than in_str')
    if mode == 'out':
        out_str += (length - cur_len - 1 - level * 2) * '-' + '+ ' + '| ' * level
    else:
        out_str += (length - cur_len - 1 - (level - 1) * 2) * ' ' + '| ' * level
    print(out_str)


def hierarchical_print_statistics(labels, total_class_num, length, level):
    non_zero_count_dict = dict()
    zero_count_dict = dict()
    for label in range(total_class_num):
        n = (labels == label).sum()
        if n > 0:
            non_zero_count_dict[label] = n
        else:
            zero_count_dict[label] = n

    key_num_per_line = 10
    key_num = len(non_zero_count_dict)
    if key_num > 0:
        hierarchical_print(in_str=f'Successful replay - '
                                  f'ClassNum-{len(non_zero_count_dict.keys())} - '
                                  f'[ClassID: Count]', length=length, mode='out', level=level)
        data_str = ''
        for idx, key in enumerate(non_zero_count_dict):
            data_str += f'{key}: {non_zero_count_dict[key]}, '
            if ((idx + 1) % key_num_per_line == 0) or (idx == key_num - 1):
                hierarchical_print(in_str=data_str, length=length, mode='in', level=level + 1)
                data_str = ''
        hierarchical_print(in_str='', length=length, mode='out', level=level)
    key_num = len(zero_count_dict)
    if key_num > 0:
        hierarchical_print(in_str=f'Unsuccessful replay - '
                                  f'ClassNum-{len(zero_count_dict.keys())} - '
                                  f'[ClassID] ', length=length, mode='out', level=level)
        data_str = ''
        for idx, key in enumerate(zero_count_dict):
            data_str += f'{key}, '
            if ((idx + 1) % (2*key_num_per_line) == 0) or (idx == key_num - 1):
                hierarchical_print(in_str=data_str, length=length, mode='in', level=level + 1)
                data_str = ''
        hierarchical_print(in_str='', length=length, mode='out', level=level)
...
class Logger(object):
    ...
    def __init__(self, fpath=None, flush=False):
